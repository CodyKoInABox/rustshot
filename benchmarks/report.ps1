...
function Format-ApplicationName([object] $Name) {
    if ([string]$Name -eq 'Rustshot') {
        return 'RustShot'
    }
    return [string]$Name
}
...
if ($rustshot -and $lightshot) {
    ...
    $rows = foreach ($application in $applications) {
        $row = @(
            (Format-ApplicationName $application.application),
            $application.version,
...
$lines.Add('')
$targetOrder = @($summary.environment.target_order | ForEach-Object {
        Format-ApplicationName $_
    }) -join ' -> '
Add-MarkdownTable $lines `
    @('Property', 'Value') `
    @(
